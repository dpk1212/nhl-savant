...
const NHLMatchupIntelligence = ({ 
  ...
  // Include locked picks even if EV < 5%
  const recommendedPlay = (bestEdge && bestEdge.evPercent > 5) || isLockedPick ? (bestEdge || {
    team: lockedBet?.bet?.pick?.split(' ')[0],
    evPercent: lockedBet?.prediction?.evPercent || 0,
    odds: lockedBet?.bet?.odds || 0,
    modelProb: lockedBet?.prediction?.modelProb || 0.5
  }) : null;
  ...
  return (
    <>
      ...
      {/* CONTEXT-AWARE MATCHUP BREAKDOWN */}
      {/* VALUE PLAY: Show opponent advantages + our value angles */}
      {/* QUALITY PLAY: Show our dominance */}
      <div style={{
        padding: isMobile ? '0.75rem 1rem' : '1rem 1.25rem',
        background: 'rgba(0, 0, 0, 0.15)'
      }}>
        {/* SECTION HEADER - Dynamic based on play type */}
        {recommendedPlay && (
          <div style={{
            fontSize: '0.875rem',
            fontWeight: '900',
            ...
            borderLeft: `3px solid ${isValuePlay ? '#D4AF37' : '#10B981'}`,
            borderRadius: '0 8px 8px 0',
            textShadow: `0 0 12px ${isValuePlay ? 'rgba(212, 175, 55, 0.5)' : 'rgba(16, 185, 129, 0.5)'}`,
            letterSpacing: '0.05em'
          }}>
            {isValuePlay ? (
              <>💰 VALUE OPPORTUNITY - Market Mispricing</>
            ) : (
              <>🔥 QUALITY PLAY - Analytical Dominance</>
            )}
          </div>
        )}

        {/* FOR VALUE PLAYS: Show "THE SETUP" (opponent advantages) */}
        {isValuePlay && recommendedPlay && (
          <div style={{
            marginBottom: '1.5rem',
            padding: '1rem',
            ...
            <div style={{
              ...
              <span style={{ fontSize: '1.063rem' }}>📊</span>
              <span>THE SETUP (Why {edgeFavoredTeam} is Favored)</span>
            </div>
            
            {/* Show opponent's advantages */}
            {factors.filter(f => f.advantage === edgeFavoredTeam).map((factor, idx) => (
              <div key={idx} style={{ marginBottom: idx < factors.filter(f => f.advantage === edgeFavoredTeam).length - 1 ? '0.75rem' : 0 }}>
                <div style={{
                  fontSize: '0.75rem',
                  color: '#EF4444',
                  fontWeight: '700',
                  ...
                  <span>{factor.name.replace('WHO CREATES BETTER CHANCES?', 'Offense').replace('WHO PREVENTS GOALS BETTER?', 'Defense').replace('WHO DOMINATES SPECIAL TEAMS?', 'Special Teams').replace('WHO HAS THE BETTER NETMINDER?', 'Goaltending')}</span>
                  <span style={{ color: '#10B981' }}>+{(factor.advantage === awayTeam ? Math.abs(awayXGF - homeXGF) : factor.advantage === homeTeam ? Math.abs(homeXGF - awayXGF) : 0).toFixed(2)}g</span>
                </div>
                {factor.keyInsight && (
                  <div style={{
                    fontSize: '0.688rem',
        ...
        {/* FOR VALUE PLAYS: Show "THE OPPORTUNITY" (our value angles) */}
        {isValuePlay && recommendedPlay && (
          <div style={{
            marginBottom: '1.5rem',
            padding: '1rem',
            ...
            <div style={{
              ...
              <span style={{ fontSize: '1.063rem' }}>💎</span>
              <span>THE OPPORTUNITY (Why {recommendedPlay.team} is the Play)</span>
            </div>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                ...
                <span>
                  <strong>Market inefficiency:</strong> {recommendedPlay.team} gets {(recommendedPlay.modelProb * 100).toFixed(0)}% win chance at {recommendedPlay.odds > 0 ? '+' : ''}{recommendedPlay.odds} odds (market implies only {((1 / (recommendedPlay.odds > 0 ? (100 / (recommendedPlay.odds + 100)) : (Math.abs(recommendedPlay.odds) / (Math.abs(recommendedPlay.odds) + 100)))) * 100).toFixed(0)}%)
                </span>
              </div>
              ...
              {Math.abs(awayPDO - 100) > 2 || Math.abs(homePDO - 100) > 2 ? (
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                  <span style={{ fontSize: '0.875rem', flexShrink: 0 }}>✓</span>
                  <span>
                    <strong>Regression edge:</strong> {edgeFavoredTeam}'s PDO {edgeFavoredTeam === awayTeam ? awayPDO.toFixed(1) : homePDO.toFixed(1)} suggests {Math.abs((edgeFavoredTeam === awayTeam ? awayPDO : homePDO) - 100) > 2 ? 'unsustainable shooting' : 'mean reversion'} ({Math.abs(awayRegressionImpact) > Math.abs(homeRegressionImpact) ? Math.abs(awayRegressionImpact).toFixed(2) : Math.abs(homeRegressionImpact).toFixed(2)}g negative impact)
                  </span>
                </div>
              ) : null}
              
              {/* Our team's strengths */}
              {factors.filter(f => f.advantage === recommendedPlay.team).length > 0 && (
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                  <span style={{ fontSize: '0.875rem', flexShrink: 0 }}>✓</span>
                  <span>
                    <strong>{recommendedPlay.team}'s advantages:</strong> {factors.filter(f => f.advantage === recommendedPlay.team).map(f => f.name.replace('WHO CREATES BETTER CHANCES?', 'offense').replace('WHO PREVENTS GOALS BETTER?', 'defense').replace('WHO DOMINATES SPECIAL TEAMS?', 'special teams').replace('WHO HAS THE BETTER NETMINDER?', 'goaltending')).join(', ')}
                  </span>
                </div>
              )}
              
              {/* Possession edge */}
              {(recommendedPlay.team === awayTeam && awayCorsi > homeCorsi + 2) || (recommendedPlay.team === homeTeam && homeCorsi > awayCorsi + 2) ? (
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                  <span style={{ fontSize: '0.875rem', flexShrink: 0 }}>✓</span>
                  <span>
                    <strong>Possession control:</strong> {recommendedPlay.team} dictates pace ({recommendedPlay.team === awayTeam ? awayCorsi.toFixed(1) : homeCorsi.toFixed(1)}% Corsi = 5-7 more shots/game)
                  </span>
                </div>
              ) : null}
            </div>
        ...
        {/* FOR QUALITY PLAYS: Show "ANALYTICAL DOMINANCE" */}
        {!isValuePlay && recommendedPlay && (
          <div style={{
            marginBottom: '1.5rem',
            padding: '1rem',
            ...
            <div style={{
              ...
              <span style={{ fontSize: '1.063rem' }}>🔥</span>
              <span>ANALYTICAL DOMINANCE</span>
            </div>
            
            {/* Show our team's advantages */}
            {factors.filter(f => f.advantage === (recommendedPlay?.team || edgeFavoredTeam)).map((factor, idx) => (
              <div key={idx} style={{ marginBottom: idx < factors.filter(f => f.advantage === (recommendedPlay?.team || edgeFavoredTeam)).length - 1 ? '0.75rem' : 0 }}>
                <div style={{
                  fontSize: '0.75rem',
                  color: '#10B981',
                  fontWeight: '700',
                  ...
                  <span>{factor.icon}</span>
                  <span>{factor.name.replace('WHO CREATES BETTER CHANCES?', 'Offense').replace('WHO PREVENTS GOALS BETTER?', 'Defense').replace('WHO DOMINATES SPECIAL TEAMS?', 'Special Teams').replace('WHO HAS THE BETTER NETMINDER?', 'Goaltending')}</span>
                  <span>+{(factor.advantage === awayTeam ? Math.abs(awayXGF - homeXGF) : factor.advantage === homeTeam ? Math.abs(homeXGF - awayXGF) : 0).toFixed(2)}g</span>
                </div>
                {factor.keyInsight && (
                  <div style={{
                    fontSize: '0.688rem',
            ...
            <div style={{
              ...
              Total Analytical Edge: +{totalEdge.toFixed(2)} goals
            </div>
          </div>
        )}

        {/* FALLBACK: No recommendation - show standard bars */}
        {!recommendedPlay && factors.map((factor, idx) => (
          <div key={idx} style={{ 
        ...
        {/* FOR VALUE PLAYS: Add "RISKS TO MANAGE" section */}
        {isValuePlay && recommendedPlay && (
          <div style={{
            marginTop: '1rem',
            padding: '1rem',
            ...
            <div style={{
              ...
              <span style={{ fontSize: '1.063rem' }}>⚠️</span>
            ...
            <div style={{
              ...
              {factors.filter(f => f.advantage === edgeFavoredTeam).map((factor, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                  <span style={{ fontSize: '0.875rem', flexShrink: 0 }}>•</span>
                  <span>
                    {edgeFavoredTeam}'s {factor.name.replace('WHO CREATES BETTER CHANCES?', 'offensive advantage').replace('WHO PREVENTS GOALS BETTER?', 'defensive edge').replace('WHO DOMINATES SPECIAL TEAMS?', 'special teams dominance').replace('WHO HAS THE BETTER NETMINDER?', 'goaltending superiority')} → {recommendedPlay.team} must {
                      factor.name.includes('CHANCES') ? 'generate high volume to overcome quality gap' :
                      factor.name.includes('PREVENTS') ? 'limit high-danger chances and turnovers' :
                      factor.name.includes('SPECIAL') ? 'stay disciplined and avoid penalties' :
                      'get strong goaltending performance'
                    }
                  </span>
                </div>
              ))}
