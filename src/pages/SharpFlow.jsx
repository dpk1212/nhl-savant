...
import ConfirmedActionDesk from '../components/sharpFlow/ConfirmedActionDesk';
import {
...
function whitelistRecordForDisplay(walletShort, sport) {
  // Shared with Action desk — stronger of Source A (picks) vs Source B (positions).
  return sportBookForDisplay(getWalletProfile(walletShort)?.bySport?.[sport]);
}
...
// Money Flow dual-bar view removed — replaced by ConfirmedActionDesk (Action tab).
...
const SharpLockCardV2 = memo(function SharpLockCardV2({
  pick, isMobile, tierWindows, pinnacleHistory,
...
const LockedPickCard = memo(function LockedPickCard({ pick, isMobile }) {
  ...
  const betOdds = (Number.isFinite(odds) && odds !== 0) ? odds
    : (Number.isFinite(lockPinnOdds) && lockPinnOdds !== 0) ? lockPinnOdds
      : null;
        ...
        for (const [gameKey, gd] of Object.entries(sportGames)) {
          ...
          for (const pos of seen.values()) {
            ...
            mktRec.wallets.push({
              ...
              roi: Number.isFinite(pos.sportROI) ? pos.sportROI : null,
