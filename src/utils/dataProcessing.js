...
export class NHLDataProcessor {
  ...
  predictTeamScore(team, opponent, isHome = false, startingGoalie = null, gameDate = null) {
    ...
    // NEW: Apply situational adjustments if schedule data available
    // Includes: B2B/rest, road trip fatigue, homecoming boost
    if (this.scheduleHelper) {
      const combinedAdj = this.scheduleHelper.getCombinedAdjustment(team, gameDate, isHome);
      if (combinedAdj !== 0) {
        totalGoals *= (1 + combinedAdj);
        
        // Get breakdown for logging
        const restAdj = this.scheduleHelper.getRestAdjustment(team, gameDate);
        const roadTripAdj = isHome ? 0 : this.scheduleHelper.getRoadTripAdjustment(team, gameDate);
        const homecomingAdj = isHome ? this.scheduleHelper.getHomecomingAdjustment(team, gameDate) : 0;
        
        // Build adjustment description
        const adjustments = [];
        if (restAdj !== 0) {
          const restType = restAdj === -0.03 ? 'B2B' : restAdj > 0 ? 'Extra Rest' : 'Rest';
          adjustments.push(`${restType} ${restAdj > 0 ? '+' : ''}${(restAdj * 100).toFixed(1)}%`);
        }
        if (roadTripAdj !== 0) {
          const awayStreak = this.scheduleHelper.getConsecutiveAwayGames(team, gameDate);
          adjustments.push(`Road Trip (game ${awayStreak + 1}) ${(roadTripAdj * 100).toFixed(1)}%`);
        }
        if (homecomingAdj !== 0) {
          const { tripLength } = this.scheduleHelper.isHomecomingGame(team, gameDate);
          adjustments.push(`Homecoming (${tripLength}-game trip) +${(homecomingAdj * 100).toFixed(1)}%`);
        }
        
        console.log(`  Situational adjustment for ${team}: ${adjustments.join(', ')} → ${totalGoals.toFixed(2)} goals`);
      }
    }
