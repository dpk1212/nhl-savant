...
function edgeBandLadderParams(pickDate) {
  if (isEdgeBandMute7Live(pickDate)) {
    return {
      muteBelow: EDGE_BAND_MUTE_BELOW_V2,
      midMult: EDGE_BAND_MID_MULT_V2,
      muteBand: 'LT7',
      muteReason: 'edge_lt7',
      midAction: 'SOFT',
      midReason: 'edge_7_10',
  ...
  return {
    muteBelow: EDGE_BAND_MUTE_BELOW_V1,
    midMult: EDGE_BAND_MID_MULT_V1,
    muteBand: 'LT5',
    muteReason: 'edge_lt5',
    midAction: 'HALF',
    midReason: 'edge_5_10',
...
function applyEdgeNetSizeOverlay({
  ...
  if (tier === 'RANK') {
    return { units: pre, action: 'HOLD', reason: 'rank_exempt', unitsPrePolicy: pre };
  }
  // Fail-open when neither metric could be scored (don't treat missing as NEITHER).
  const hasEdge = edge != null && Number.isFinite(Number(edge));
  const hasNet = net != null && Number.isFinite(Number(net));
  if (!hasEdge && !hasNet) {
    return { units: pre, action: 'HOLD', reason: 'fail_open_no_metrics', unitsPrePolicy: pre };
  }
  if (bucket === 'BOTH') {
    let out = Math.min(unitCap, pre * EDGE_NET_BOTH_MULT);
    ...
    return { units: out, action: 'BOOST', reason: 'edge_net_both', unitsPrePolicy: pre };
  }
  if (bucket === 'ONE') {
    return { units: pre, action: 'HOLD', reason: null, unitsPrePolicy: pre };
  }
  // NEITHER
  if (EDGE_NET_SOFT_NEITHER_TIERS.has(tier)) {
    let out = Math.round(pre * EDGE_NET_NEITHER_SOFT_MULT * 100) / 100;
    if (typeof oddsCapFn === 'function') out = oddsCapFn(out, odds);
    out = Math.max(0, Math.round(out * 100) / 100);
    return { units: out, action: 'HALF', reason: 'edge_net_neither_soft', unitsPrePolicy: pre };
  }
  return { units: pre, action: 'HOLD', reason: null, unitsPrePolicy: pre };
}
/**
...
function applyEdgeBandSizeOverlay({
  ...
  if (e < EDGE_BAND_BOOST_AT) {
    let out = Math.round(pre * ladder.midMult * 100) / 100;
    if (typeof oddsCapFn === 'function') out = oddsCapFn(out, odds);
    out = Math.max(0, Math.round(out * 100) / 100);
    return {
      units: out, action: ladder.midAction, band: 'MID', reason: ladder.midReason, unitsPrePolicy: pre,
    };
  }
...
function applySkillFeatureStamps(target, bundle, now, {
  ...
  if (flinchFailOpenAction != null) target.v8_flinchFailOpenAction = flinchFailOpenAction;
  ...
  if (steamTailAction != null) target.v8_steamTailAction = steamTailAction;
  ...
  if (unitTierEvSteamAction != null) target.v8_unitTierEvSteamAction = unitTierEvSteamAction;
  ...
  if (hardAgMuteAction != null) target.v8_hardAgMuteAction = hardAgMuteAction;
  ...
  if (steamTailBand != null) target.v8_steamTailBand = steamTailBand;
...
function consensusLine(positions, side, sport = null, marketType = null) {
  ...
  for (const p of rows) {
    ...
    const cur = counts.get(ln) || { n: 0, invested: 0 };
    cur.n += 1;
    cur.invested += Number(p.invested) || 0;
    counts.set(ln, cur);
  }
...
/**
 * Size-weighted Polymarket avgPrice → American odds for a side.
...
/** @deprecated alias — spreads + totals share the same Poly→American path. */
const consensusSpreadOddsFromPoly = consensusOddsFromPoly;
...
async function createMissingLockedPicks({
  ...
  for (const [groupKey, positions] of groups.entries()) {
    ...
    for (const side of sides) {
      ...
      // ML/SPREAD/TOTAL without a ticket price — never create a staked lock at 0 odds.
      // Wait for the next cycle once pinnacle_history / poly has a quote.
      if ((marketType === 'ML' || marketType === 'SPREAD' || marketType === 'TOTAL')
      ...
      if (isTapeSizingLive(TARGET_DATE)) {
        // RANK / CONFIRMED-UNOPP / CONFIRMED-Q1 exempt from tape mute.
        ...
        if (rankTapeMuteExempt) {
          const tapeExemptReason = hcStakeTierCreate === 'CONFIRMED-Q1'
            ? 'confirmed_q1_tape_mute_exempt'
            : (hcStakeTierCreate === 'CONFIRMED-UNOPP'
      ...
      // Ev-drift × EDGE mute — after crowded, before climate. EDGE≥15 ∧
      // dEv≤−1.5 → 0u. Steam-tail (T) is the last size overlay. On create
      // there is no prior first Ev → HOLD (fail-open). Fires on later
      // reconcile cycles once the tape log has a first sample.
      let evDriftPolicyCreate = null;
      let evDriftCreate = { firstEv: null, currentEv: null, dEv: null };
      let liveTapeCreate = null;
      let tapeCreateCtxEarly = null;
      if (createV121Eligible && peakUnitsApplied > 0) {
      ...
      // Sport Confirmed unlock CAP — last CAP before steam-tail. NFL/CFB only.
      ...
      if (createV121Eligible && peakUnitsApplied > 0) {
        ...
        steamTailPolicyCreate = applySteamTailPolicyFromTicket({
          units: peakUnitsApplied,
          pickDate: TARGET_DATE,
          walletDetails,
          side,
          ...
          hasPinnGame: !!tapeCreateCtxEarly.pinnGame,
      ...
      if (createV121Eligible && peakUnitsApplied > 0) {
        ...
        unitTierPolicyCreate = applyUnitTierEvSteamOverlay({
          units: peakUnitsApplied,
      ...
      // HARD+ AG mute — after HARD exception, before operator kill.
      // Get out of the way when a HARD wallet is on the other side.
      // Remutes both-sides tickets the exception just put back.
      // Fail-open HOLD if byMarket schema is missing. 4u+ not exempt.
      let hardAgPolicyCreate = null;
      if (createV121Eligible) {
        hardAgPolicyCreate = applyHardAgMuteOverlay({
          units: peakUnitsApplied,
          marketType,
          sport,
          side,
      ...
      if (createV121Eligible) {
        ...
        if (hcStakeTierCreate === 'CONFIRMED-Q1') {
          v8Stamps.v8_confirmedQ1Promote = true;
        }
      ...
      if (createV121Eligible && Array.isArray(walletDetails) && walletDetails.length > 0) {
        ...
        applySkillFeatureStamps(v8Stamps, skillCreate, now, {
          ...
          unitsPreFoolsGold: (foolsGoldPolicyCreate && Number.isFinite(foolsGoldPolicyCreate.unitsPrePolicy))
            ...
            : null,
          flinchFailOpenAction: flinchFailOpenPolicyCreate?.action ?? null,
          ...
          unitsPreClimate: (climatePolicyCreate && Number.isFinite(climatePolicyCreate.unitsPrePolicy))
            ? climatePolicyCreate.unitsPrePolicy
            : null,
          steamTailAction: steamTailPolicyCreate?.action ?? null,
          ...
          unitsPreFavJuice: (favJuicePolicyCreate && Number.isFinite(favJuicePolicyCreate.unitsPrePolicy))
            ...
            : null,
          unitTierEvSteamAction: unitTierPolicyCreate?.action ?? null,
          ...
          hardAgMuteAction: hardAgPolicyCreate?.action ?? null,
          ...
          steamTailSharpAB: steamTailPolicyCreate ? !!steamTailPolicyCreate.sharpAB : null,
          steamTailBand: steamTailPolicyCreate?.band ?? null,
...
function reconcileSide({ sd, side, pick, mkt, group, walletProfiles, now, force, agsCalibration, isProvenFn, isHcEligibleFn, walletStatsFn, walletPriorStatsFn, gameMeta = null, sportWinnerBoards = null, clvLedger = null, qConvMuteThr = null, pathBlendPriors = null, expWinPriors = null }) {
  ...
  if (isConfirmedQ1PromoteLive(pickDate) && appliedStatus === 'ACTIVE'
    ...
    if (finalUnitsApplied < tgt) {
      finalUnitsApplied = tgt;
  ...
  // After HC / RANK / SHARP / Q1 / CONFIRMED-UNOPP leave a pick at 0u, rescue when
  ...
  if (appliedStatus === 'ACTIVE' && finalUnitsApplied > 0 && !skipManualSize) {
    if (isEdgeBandApplicable(hcStakeTier, pickDate)) {
  ...
  if (tapeSizingLive) {
    ...
    if (rankTapeMuteExempt) {
      const tapeExemptReason = hcStakeTier === 'CONFIRMED-Q1'
        ? 'confirmed_q1_tape_mute_exempt'
        : (hcStakeTier === 'CONFIRMED-UNOPP'
          ? 'confirmed_unopp_tape_mute_exempt'
          : 'rank_tape_mute_exempt');
  ...
  // ─── Ev-drift × EDGE mute (after crowded, before climate) ────────────
  ...
  // ─── Sport Confirmed unlock CAP (last CAP before steam-tail) ──────────
  ...
  if (v121Eligible && finalUnitsApplied > 0 && !skipManualFlinch) {
    ...
    steamTailPolicy = applySteamTailPolicyFromTicket({
      units: finalUnitsApplied,
      pickDate,
      walletDetails: wd,
      side,
      ...
      hasPinnGame: !!tapeCtxLive.pinnGame,
  ...
  if (v121Eligible && finalUnitsApplied > 0 && !skipManualFlinch) {
    ...
    unitTierPolicy = applyUnitTierEvSteamOverlay({
      units: finalUnitsApplied,
  ...
  // HARD+ AG mute — after HARD exception, before odds-cap / operator kill.
  // Get out of the way when a HARD wallet is on the other side.
  // Remutes both-sides tickets the exception just put back.
  // Fail-open HOLD if byMarket schema is missing. Manual stake exempt.
  // 4u+ is not exempt.
  let hardAgPolicy = null;
  if (v121Eligible && !skipManualFlinch) {
    hardAgPolicy = applyHardAgMuteOverlay({
      units: finalUnitsApplied,
      marketType: mkt,
      sport: pick.sport,
      side,
  ...
  if (agsV12Result) {
    ...
    if (v121Eligible) {
      ...
      if (confirmedQ1Rescued || hcStakeTier === 'CONFIRMED-Q1') {
        patch.v8_confirmedQ1Promote = true;
      } else if (sd.v8_confirmedQ1Promote != null) {
        patch.v8_confirmedQ1Promote = admin.firestore.FieldValue.delete();
      }
  ...
  if (tapePolicy?.reason === 'rank_tape_mute_exempt'
      ...
      || tapePolicy?.reason === 'confirmed_q1_tape_mute_exempt') {
    const who = tapePolicy.reason === 'confirmed_q1_tape_mute_exempt'
      ? 'CONFIRMED-Q1'
      : (tapePolicy.reason === 'confirmed_unopp_tape_mute_exempt' ? 'CONFIRMED-UNOPP' : 'RANK');
  ...
  if (flinchFailOpenPolicy?.action === 'MUTE') {
    changes.push(
      `FLINCH-MUTE: ${flinchFailOpenPolicy.reason} `
      + `tape=${tapePolicy?.action || clvPolicy?.action || '—'} `
      + `E=${winnerAlign?.edge == null ? '—' : Number(winnerAlign.edge).toFixed(1)} `
  ...
  if (steamTailPolicy?.action === 'MUTE') {
    changes.push(
      `STEAM-TAIL-MUTE: ${steamTailPolicy.reason || 'steam-tail'} `
      + `band=${steamTailPolicy.band || '—'} `
      + `ab=${steamTailPolicy.sharpAB ? 'Y' : 'N'} `
      + `steam=${steamTailPolicy.steamOnLock ? 'ON' : 'off'} `
      + `arriving=${steamTailPolicy.steamArriving ? 'Y' : 'N'} `
      + `${steamTailPolicy.unitsPrePolicy}u → 0u (${hcStakeTier})`
    );
  }
  if (steamTailPolicy?.action === 'FLOOR') {
    changes.push(
      `STEAM-TAIL-FLOOR: arriving A/B `
  ...
  if (confirmedQ1Rescued) {
    changes.push(
      `CONFIRMED-Q1${confirmedQ1Floored ? '/FLOOR' : ''}: forQ1=${confirmedQ1Slice.forQ1Sized} `
      + `bestSize=${confirmedQ1Slice.bestSize ?? '—'}× `
      + `→ ${confirmedQ1Slice.targetUnits}u (${confirmedQ1Slice.wallets.join('+') || '—'})`
    );
  }
  ...
  if (v121Eligible) {
    ...
    applySkillFeatureStamps(patch, skillLive, now, {
      ...
      unitsPreTape: (tapeSizingLive && tapePolicy && Number.isFinite(tapePolicy.unitsPrePolicy))
        ...
        : null,
      edgeNetSizeAction: edgeNetSizePolicy?.action ?? null,
      ...
      steamTailSharpAB: steamTailPolicy ? !!steamTailPolicy.sharpAB : null,
      steamTailBand: steamTailPolicy?.band ?? null,
  ...
  // LowVig/etc. Re-stamp missing odds pre-T-15 so cards aren't stuck at 0.
  {
    ...
    // TOTAL line/odds — wallet instrument (mirrors SPREAD repair below).
    // Only backfill when lock/peak line is missing or junk (≤1). Never rewrite
    // a real vault entryLine (7.5/8.5) onto live sportsbook MAIN (9.5).
    if (mkt === 'TOTAL' && pick.status !== 'COMPLETED') {
      ...
      if (Number.isFinite(fillLine) && isPlausibleLine(fillLine, sport, 'TOTAL')) {
        ...
        const lockJunk = Number.isFinite(lockLn) && !isPlausibleLine(lockLn, sport, 'TOTAL');
        const peakJunk = Number.isFinite(peakLn) && !isPlausibleLine(peakLn, sport, 'TOTAL');
        const lockLineMissing = !Number.isFinite(lockLn) || lockJunk;
        const peakLineMissing = !Number.isFinite(peakLn) || peakJunk;
        if (lockLineMissing) {
          patch.lock = {
            ...(patch.lock || {}),
            line: fillLine,
        ...
        if (peakLineMissing) {
          ...
          changes.push(`totalPeakLine backfill: ${fillLine}`);
        }
        // Odds-only repair when line already set but create burned -110 / 0.
        // Skip when stamped line is a wallet alt ≠ MAIN — Poly odds are correct.
        ...
        const isWalletAlt = Number.isFinite(stampedLn) && Number.isFinite(fillLine)
          && Math.abs(stampedLn - fillLine) > 0.051;
        ...
        if (!isWalletAlt && Number.isFinite(fillOdds) && !lockLineMissing
        ...
        if (!isWalletAlt && Number.isFinite(fillOdds) && !peakLineMissing
  ...
  // Pre-T-15 TOTAL: rewrite MAIN-stamped tickets onto the vault entryLine
  // (SEA Over 8.5 @ Poly while books moved to 9.5). Mirrors SPREAD repair.
  if (pick.status !== 'COMPLETED' && mkt === 'TOTAL' && Array.isArray(group) && group.length) {
    const metaKey = `${sport}|${pick.gameKey}`;
    const meta = gameMeta?.get?.(metaKey) || null;
...
async function main() {
  ...
  FLAT_DOLLAR_Q_BY_SPORT = buildFlatDollarQBySport(walletProfiles);
  {
    const q1n = [...FLAT_DOLLAR_Q_BY_SPORT.entries()]
  ...
  if (isFlinchFailOpenMuteLive(TARGET_DATE)) {
    console.log(
      `Flinch/fail-open leftover mute LIVE: still <4u AND (odds-capped native-4u OR tape BOOST OR E≥10 OR FAIL_OPEN) → 0u`
      + ` · from ${FLINCH_FAIL_OPEN_MUTE_FROM} · 4u+ never touched`,
    );
  } else {
  ...
  if (isEvDriftEdgeMuteLive(TARGET_DATE)) {
    console.log(
      ...
      + ` · from ${EV_DRIFT_EDGE_MUTE_FROM} · before climate · missing Ev fail-open · mutedBy=${EV_DRIFT_EDGE_MUTED_BY}`,
    );
  } else {
  ...
  if (isConfirmedQ1PromoteLive(TARGET_DATE)) {
    console.log(
      ...
      + `→ ${CONFIRMED_Q1_UNITS}u (${CONFIRMED_Q1_PRESS_UNITS}u if size≥1×) · opposed OK`
      + ` · from ${CONFIRMED_Q1_FROM} · hard floor after mutes`,
    );
  } else {
    console.log(`CONFIRMED-Q1 promote: not live before ${CONFIRMED_Q1_FROM} (TARGET_DATE=${TARGET_DATE})`);
  }
