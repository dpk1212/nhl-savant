...
function buildV12TierAnalysis(report, stats) {
  ...
  if (v121Rows.length > 0) {
    ...
    report.push('');
    report.push(`Post-cutover picks size off the **HC margin** — SUPER (margin 2 · 6u), TOP (margin 1 · 4u), MINI (mini-HC 1.0–1.5× · 3u), CONFIRMED (margin 3+ · 1u) — **plus** the **RANK (2-for-0)** wallet-rescue path at **${RANK_RESCUE_UNITS}u**. From **2026-06-26** the **v12abc proven-$ overlay** (internal stats: backer \`positions.dollarRoi\` + featured \`picks.wr\`) adds: **SHARP / SHARP-PRIME** ($-rescue of HC-muted picks at 3u / 4u when ≥3 sharps back it incl. a proven-money winner, mean win-rate ≥ 50 / 55, and odds softer than −150), and **MINI-** (MINI cut 3u → 1u when no proven-$ backer is on it). **TOP+** (HC-1 4u→5u boost) is calibrated **OFF** after live Path C ran −9% ROI on the boost. Together these paths ARE the v12abc staked book. **MONITORING** (non-HC or WEAK-tier HC, no proven-$ rescue) is tracked at **0u** and excluded from the staked record/ROI below.`);
    report.push('');
