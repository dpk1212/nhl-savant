...
(async () => {
  ...
  // ── 6. DAILY PnL BY FACTOR BUCKET ────────────────────────────────
  // For every significant factor, render a matrix with
  //   rows = bucket values, cols = each day + TOTAL
  //   cell = "N · WR% · flatROI% · flatPnL u"
  // Answers "did this slice hit or miss consistently, or is the total
  // just one lucky/unlucky day?"
  out.push('\n---\n');
  out.push('## 6. Daily PnL by factor bucket (stability check)');
  out.push('');
  out.push('Each row shows how one bucket performed **per day** + total. Look for signals that are *consistent* across days (stable edge) vs ones that flip signs day-to-day (small-sample noise).');
  out.push('');
  out.push(`Cell format: **N · WR% · flatROI% · flatPnL**`);
  out.push('');

  const dateCols = [...byDate.keys()].sort();

  function matrixCellCompact(a) {
    if (!a || !a.n) return '—';
    return `N=${a.n} · ${a.wr.toFixed(0)}% · ${sign(a.flatRoi, 0)}% · ${sign(a.flatPnl, 1)}u`;
  }

  function dailyMatrix(title, rowFn) {
    const buckets = [...new Set(all.map(rowFn))].filter(b => b != null).sort();
    const headers = ['Bucket', ...dateCols.map(d => d.slice(5)), 'TOTAL'];
    const lines = [];
    lines.push(`### ${title}`);
    lines.push(mdHeader(headers));
    for (const b of buckets) {
      const inBucket = all.filter(r => rowFn(r) === b);
      const row = [b];
      for (const d of dateCols) {
        row.push(matrixCellCompact(agg(inBucket.filter(r => r.date === d))));
      }
      row.push(`**${matrixCellCompact(agg(inBucket))}**`);
      lines.push(`| ${row.join(' | ')} |`);
    }
    lines.push('');
    return lines.join('\n');
  ...
  for (const [name, fn] of factors) {
    out.push(dailyMatrix(name, fn));
  }

  // Quick read-out: which buckets are positive every day?
  out.push('### 6a. Stability scoreboard');
  out.push('');
  out.push('Which buckets posted **positive flat ROI on every day they appeared** (min 2 days of appearances)? These are the most trustworthy slices.');
  out.push('');
  out.push(mdHeader(['Factor', 'Bucket', 'Days appeared', 'Days positive', 'TOTAL']));
  const stability = [];
  for (const [fname, fn] of factors) {
    const buckets = [...new Set(all.map(fn))].filter(b => b != null);
    for (const b of buckets) {
      const inBucket = all.filter(r => fn(r) === b);
      const daysPresent = dateCols.filter(d => inBucket.some(r => r.date === d));
      if (daysPresent.length < 2) continue;
      ...
      stability.push({
        ...
        perfect: daysPositive.length === daysPresent.length,
      });
    }
  }
  const perfect = stability.filter(s => s.perfect && s.totalRoi > 0).sort((a, b) => b.totalRoi - a.totalRoi);
  perfect.forEach(s => {
    out.push(`| ${s.fname} | ${s.b} | ${s.present} | ${s.positive} | N=${s.totalN} · ${sign(s.totalRoi)}% · ${sign(s.totalPnl, 2)}u |`);
  });

  out.push('');
  out.push('Which buckets posted **negative flat ROI on every day they appeared** (min 2 days)? These are the most trustworthy bleeders.');
  out.push('');
  out.push(mdHeader(['Factor', 'Bucket', 'Days appeared', 'Days negative', 'TOTAL']));
  const allNegative = stability.filter(s => s.positive === 0 && s.totalRoi < 0).sort((a, b) => a.totalRoi - b.totalRoi);
  allNegative.forEach(s => {
    out.push(`| ${s.fname} | ${s.b} | ${s.present} | ${s.present - s.positive} | N=${s.totalN} · ${sign(s.totalRoi)}% · ${sign(s.totalPnl, 2)}u |`);
  });
  out.push('');
