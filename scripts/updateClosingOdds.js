...
async function run() {
  ...
  const q = query(collection(db, 'sharpFlowPicks'), where('status', '==', 'PENDING'));
  ...
  for (const d of snap.docs) {
    ...
    try {
      await updateDoc(doc(db, 'sharpFlowPicks', d.id), updates);
      updated++;
    } catch (e) {
      console.warn(`  ⚠️  Failed to update ${d.id}: ${e.message}`);
      skipped++;
    }
  }

  console.log(`✅ Closing odds: ${updated} picks updated, ${skipped} skipped (${snap.size} total PENDING)`);
  process.exit(0);
}
