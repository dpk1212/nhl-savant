...
check('Central Arkansas is CARK not Arkansas', resolveCFBTeam('Central Arkansas') === 'CARK');
check('North Carolina Central is NCCU not UNC', resolveCFBTeam('North Carolina Central') === 'NCCU');
check('Eastern Washington is EWU not Washington', resolveCFBTeam('Eastern Washington') === 'EWU');
check('Texas Southern is TXSO not Texas', resolveCFBTeam('Texas Southern') === 'TXSO');
check('Northwestern (IA) is not Northwestern', resolveCFBTeam('Northwestern (IA)') == null);
check('Florida A&M is FAMU not Florida', resolveCFBTeam('Florida A&M') === 'FAMU');
check('Southern Miss alias', resolveCFBTeam('Southern Miss') === 'USM');
check('Louisiana-Monroe is ULM not ULL', resolveCFBTeam('Louisiana-Monroe') === 'ULM');
check('Tennessee-Martin is UTM not TENN', resolveCFBTeam('Tennessee-Martin') === 'UTM');
check('Virginia-Lynchburg is not UVA', resolveCFBTeam('Virginia-Lynchburg') === 'VLB');
check('Tennessee still TENN', resolveCFBTeam('Tennessee') === 'TENN');
check('Louisiana still ULL', resolveCFBTeam('Louisiana') === 'ULL');
check('Virginia still UVA', resolveCFBTeam('Virginia') === 'UVA');
check('Mercer vs Georgia Tech key', makeCFBGameKey('Mercer', 'Georgia Tech') === 'mer_gt');
check('UConn vs Southern Miss key', makeCFBGameKey('UConn', 'Southern Miss') === 'conn_usm');
check('SIU vs Illinois key', makeCFBGameKey('Southern Illinois', 'Illinois') === 'siu_ill');
check('Montana vs Oregon State key', makeCFBGameKey('Montana', 'Oregon State') === 'mont_orst');
check('South Dakota vs Boise State key', makeCFBGameKey('South Dakota', 'Boise State') === 'sdak_bsu');
check('North Dakota vs Nebraska key', makeCFBGameKey('North Dakota', 'Nebraska') === 'und_neb');
check('Penn is not Penn State', resolveCFBTeam('Penn') === 'PENN' && resolveCFBTeam('Penn State') === 'PSU');
check('San Diego is not SDSU', resolveCFBTeam('San Diego') === 'USD' && resolveCFBTeam('San Diego State') === 'SDSU');
check('Montana is not Montana State', resolveCFBTeam('Montana') === 'MONT' && resolveCFBTeam('Montana State') === 'MTST');
